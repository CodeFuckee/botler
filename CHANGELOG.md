...
- **设置页视觉精修（Geist 体系内，用户反馈美化需求）**：以设置页为主、
  少量全局微调，全部走现有 token，无新依赖、零行为改动；明暗两套主题
  逐张截图回归（13 页面 × 7 视口 + 设置页暗色）：
  - 卡片/板块标题（全局 `h2`）：由「全大写灰字」改为句首大写主文本色
    （15px/600，负字距），层级靠字重与颜色深浅表达；设置分组眉标
    （`.settings-group-title`）保留全大写形成反差；
  - 设置项行（`.table.kv`）：label 升为主文本（13px 主色文字，真实表头
    仍为灰色 12px），键名 `code` 独立成行弱化为 11px 灰阶徽章，行距
    9px→12px 增加呼吸感（任务详情等 kv 表格同步受益）；
  - 布尔开关（`.check-input`）：原生 checkbox 以 `appearance: none`
    自绘为「轨道 + 滑块」开关（36×20，选中主色轨道）——DOM 与交互行为
    不变（点击/键盘/表单语义一致），新增 `:focus-visible` 焦点环保留
    键盘可达性；快捷键弹窗、任务页「显示用量」等同步升级；
  - 设置导航栏：选中态指示条改为 inset 阴影（随圆角弯曲），选中项加
    500 字重；分组计数徽章数字居中等宽；搜索框融入页面底色；
  - 设置卡片间距 16px→24px（仅设置内容区，`.settings-content .card`），
    长卡片列表呼吸感更强；
  - 折叠态精修（用户截图反馈）：设置导航整体折叠后的窄栏展开按钮从
    裸 `»` 字形升级为与全局侧栏一致的 28px 卡片按钮（Lucide
    `chevronsRight` 图标，hover/focus 态齐全）；全局侧栏折叠态的机器人
    标识放入与展开按钮同宽的 28px 居中容器并放大到 18px，头部收成
    整齐的居中一列；
  - 设置导航折叠态二次精修（用户反馈「展开箭头占地方」）：折叠不再
    保留 44px 独立窄栏——aside 收为零宽、栅格间距归零，内容区取回
    全部宽度；展开入口改为幽灵图标（无底无框 muted 箭头）悬浮在
    内容区左上角、与首个分组标题同一行，滚动时保持吸顶可见；
    折叠态 sticky top 68→24px 消除与标题约 40px 的错位。
...
- **修复统计页未撑满窗口宽度（issue #474）**：：
  ...
  - 根因：`.stats-page` 样式固定 `max-width: 1080px`，而全局内容容器
    ...
    calc(100vw - 100px))`（1920px 视口约 1820px），统计页被 1080px
    上限卡住——其余页面（概览/任务等）均跟随容器宽度，唯独统计页留白；
  - 修复：移除 `.stats-page` 的 `max-width: 1080px` 限制，改为
    `width: 100%` 跟随 `.content` 容器宽度，与概览/任务等其他页面一致
    的响应式布局；内部卡片 grid（auto-fit）/ 表格（width:100%）/ 图表
    （各自保持 `max-width: 720px` 合理观感）随容器自适应；
  - 测试：`frontend/tests/stats-page.test.mjs` 新增源码断言——`.stats-page`
    规则块不得再含固定 `max-width`（修复前失败 / 修复后通过）。
...
- **灵感批量转 issue（多选 + 转前预览编辑）（issue #247）**：
  - 需求：灵感转 issue 此前只能一条条操作（点「添加 Issue」→ 确认），
    ...
    （现状描述原文当标题，超长会被截断，issue #186 修了截断但内容仍是
    原文）；
  - 后端：新增 `POST /api/inspirations/batch-add-issues` 批量接口——
    请求 `items` 每条含 `inspiration_id` 与可选覆盖字段（`title` /
    `description` / `labels` / `repo_id`），逐条调用与单条 add-issue
    完全一致的核心逻辑（抽取出 `_submit_inspiration_as_issue` 供单条/
    批量共用，保证「单条转 issue 行为不变」）；单条失败不阻断其余条目
    （灵感不存在 / 仓库禁用 / 标题或标签覆盖非法 / owner token 未配置 /
    GitLab 故障等均记为该条失败并继续），成功项按 issue #162 语义删除
    灵感、失败项保留可重试；响应含 `succeeded` / `failed` 数组与
    `summary`（成功/失败计数），前端据此展示「N 成功 / M 失败」汇总；
    同一灵感 id 重复出现去重（只处理首次），单次上限 50 条；
  - 前端：灵感板块标题行新增「多选」按钮——进入多选模式后每条灵感前
    出现复选框、板块顶部出现批量工具栏（已选计数 +「转为 Issue」+ 取消，
    未选中时「转为 Issue」禁用）；点击「转为 Issue」弹出批量预览弹窗，
    每条可单独编辑标题/描述/标签（逗号分隔）/目标仓库，或「全部应用
    默认」统一重置（默认标题=描述=灵感内容、标签 feature+ui、目标仓库=
    灵感所属仓库）；逐条提交后展示「N 成功 / M 失败」汇总与逐条失败原因，
    失败条目在面板内标红原因、成功条目由后端删除后列表即时刷新；
  - 测试：后端 `test_api_inspirations.py` 新增 `TestBatchAddIssueFrom
    Inspirations` 14 用例（正常/覆盖/截断/去重/空 items/超限/部分失败
    隔离/仓库覆盖禁用/未配置 owner token/缓存失效等）；前端新增
    `frontend/tests/overview-inspirations-batch.test.mjs` 11 用例
    （源码断言 + 多选交互 + 预览编辑 + payload 覆盖字段 + 汇总展示 +
    失败保留/取消退出等）；全量测试无回归。


  - 需求：概览页开放 issue 在现有「仓库卡片」布局（每个仓库一张卡片、卡片内按
    bot 状态分组、宽屏多列网格）之外，新增一种布局方式——所有仓库的 issue 在
    同一列展示，同一个仓库的 issue 归为一个分组，分组可以折叠/展开；
  - 实现：过滤条顶部新增「布局」切换（仓库卡片 / 单列分组，与排序/状态/标签
    过滤同风格按钮组），布局偏好存 localStorage（键 `botler.overview.layout`），
    刷新后保持；「单列分组」布局下所有仓库 issue 单列 grid 展示，每个仓库一个
    分组卡片，组头复用 `.issue-repo-head`（折叠开关 + 仓库名 + 优先级 + 计数 +
    对账/自省/发掘/添加 Issue 按钮），组内平铺该仓库全部 issue（保持当前排序/
    过滤结果，issue 项复用卡片布局渲染：bot 状态徽章、运行中高亮、标签/里程碑、
    assignee/更新时间/评论数、置顶按钮、任务信息块、批量勾选）；
  - 折叠偏好存 localStorage（键 `botler.overview.collapsedRepos`），折叠隐藏组内
    issue 列表、保留组头与操作反馈；分组可独立折叠、互不影响；过滤/排序/批量
    关闭在两种布局下共用；拖动排序仅卡片布局「其他」分组启用（手动顺序语义为
    仓库内「其他」分组，单列布局平铺视图下不启用，置顶按钮不受影响）；
  - 测试：新增 `overview-layout-column.test.mjs` 19 用例（布局/折叠偏好纯函数
    解析规范化边界 + 渲染交互：切换布局、组折叠展开、localStorage 持久化与预置
    生效、操作按钮保留）；`responsive-mobile-layout` 契约更新允许
    `.issues-list-column` 专属单列类（用户主动切换布局，非响应式覆盖）；
  - 前端全量单测 1732 用例通过、eslint 通过、`npm run build` 通过。

- **灵感 AI 对话支持按记录选择供应商/模型（issue #249）**：
  - 对话抽屉顶部新增「回复模型」下拉，只列出已启用且 API Key 非空的 AI API 供应商，并以 `provider / model · 名称` 标注；未配置时提供前往设置页的引导；
  - `inspirations.chat_provider` 通过 SQLite v28 迁移持久化选择，重新打开同一灵感仍保持；切换供应商不会删除历史消息，后续回复携带完整既有上下文；
  - 新增供应商查询/保存接口，发送消息接口增加可选 `provider` 参数；显式选择不可用时拒绝请求，已保存供应商后来被停用或删除时自动回退首个可用项，兼容旧客户端默认行为；
  - 测试覆盖可用项过滤、持久化/清除、无配置、无效参数、失效回退、指定模型路由和历史上下文保留，以及前端下拉/设置页引导。
...
- **设置页左侧设置导航栏搜索框去掉搜索图标（issue #442）**：设置页左侧导航栏的搜索框不再渲染放大镜图标，输入框直接占满搜索容器整行；搜索过滤、占位文案与清除按钮等交互能力保持不变（仅去图标，不砍功能）。同步移除 `styles.css` 中 `.settings-nav-search-icon` 样式规则，新增源码 / 样式 / E2E 三层回归断言确保图标不回归且搜索仍可用。

- **概览页灵感组件「AI 对话抽屉」输入框优化为 codex 风格（issue #443）**：文本框与发送按钮融合为单一圆角 composer 容器，聚焦时容器整体主色描边 + 焦点环高亮；文本框无独立边框、`rows=1` 随内容自动增高，达到上限（160px）后内部滚动；发送按钮改为圆形上箭头图标按钮（带国际化 `aria-label` / `title`，无可见文本），空输入或发送中禁用；Enter 发送、Shift+Enter 换行、发送失败输入保留等交互保持不变。新增源码级 / 渲染级 / 交互级测试 `frontend/tests/overview-chat-codex-input.test.mjs`（含自动增高上限截断等边界用例）。
...
- **README 首次部署与快速上手文档（issue #428）**：补充 Botler 定位、核心能力、典型使用流程、运行环境、GitLab PAT / 执行引擎 API Key 配置边界、可直接运行的本地启动示例、开发参与入口及许可证说明；同步澄清执行完成仅追加结果评论和 `bot-done` 标签，Issue 关闭由人工确认。

### Changed

- **折叠态左侧边栏展开按钮调整至应用图标上方（issue #435）**：桌面端侧边栏收起后，头部改为纵向居中排列，展开按钮通过排序置于 Botler 应用图标上方，不再显示在图标右侧；展开态与窄视口抽屉导航维持原有布局。新增对应 CSS 布局回归测试。
...
- **Issue 评论支持上传图片附件（issue #432）**：概览页 Issue 详情右边栏的评论输入区新增“选择图片”按钮，支持 PNG、JPEG、GIF、WebP 图片；提交评论时图片先上传至对应 GitLab 项目，再将 GitLab 返回的 Markdown 图片引用与评论正文一并发布，因此图片可在 Botler 与 GitLab Issue 评论中同步展示。支持移除待上传图片、空文件/非图片拦截和上传失败后保留原评论内容重试。

- **GitLab API 全局限速与对账抖动（issue #195）**：所有 `GitLabClient` 现在共享可配置的匀速请求通道（默认 10 req/s），webhook、定时对账、全量手动对账及 429 重试均不会绕过限速；全量对账还会在相邻启用仓库间加入默认 0.5～2 秒随机抖动，避免多仓库 API 请求集中爆发。429 保持指数退避重试并在日志中明确记录限流原因。新增限速、重试通道和抖动的回归测试。

- **概览页创建 Issue 后自动触发仓库对账（issue #425）**：在概览页「添加 Issue」弹窗成功创建 Issue 后，前端会串行调用对应仓库的对账接口，使新分配给 bot 且尚无活跃任务的 Issue 无需等待定时扫描即可入队；自动对账失败不会回滚或误报已成功的创建操作，页面仍刷新开放 Issue 列表，并在对应仓库卡片显示可手动重试的失败提示。新增创建→对账调用顺序及失败降级回归测试。

### Changed

- **所有执行引擎取消 1800 秒任务时限（issue #424）**：Claude、Hermes 与 DSH
  执行路径不再创建任务 deadline，也不会因 `task_timeout_seconds` 自动中断；任务仅在
  正常完成或人工停止时结束。设置页与仓库编辑弹窗移除已失效的超时配置入口，任务列表
  和详情统一展示“未限制”；历史 `worker.task_timeout_seconds`、仓库
  `timeout_seconds` 配置仅兼容读取且不会恢复限时行为。配置示例、README、设计文档及
  回归测试同步更新。

### Added

- **概览页默认隐藏批量关闭勾选框（issue #414）**：开放 Issue 列表默认只展示「批量关闭」入口，不再让全选和每条 Issue 的勾选框干扰日常浏览；点击入口后进入选择模式，才显示全选、逐项勾选、已选数量和确认关闭按钮。新增「取消」操作，退出选择模式时清空临时选择；原有逐条关闭、二次确认、部分失败归集及重复提交防护保持不变，并新增默认/进入/取消模式回归测试。
...
- **设置页左侧导航搜索框图标与提示文字重叠（issue #436）**：固定搜索图标为 16px，并调整至距左侧 12px；同时将输入框左内边距扩至 40px，为图标右侧保留 12px 间距，避免提示文字与图标重叠。新增样式回归测试。

- **概览页开放 Issue 触屏行元信息上下错位（issue #434）**：标题按钮在触屏设备中因最小触控目标扩展为 44px，右侧的处理人头像、更新时间及评论图标/数量仍按紧凑行顶部排列，导致视觉中心偏上。现让右侧信息容器在粗指针设备采用同样的 44px 行高并垂直居中，桌面端紧凑布局保持不变；新增桌面和竖屏触屏的浏览器几何回归测试。

- **流水线测试报告改为抽屉全局垂直滚动（issue #433）**：移除报告明细列表的固定高度与局部 `overflow-y`，统一由流水线详情抽屉滚动，避免长测试报告出现嵌套滚动区域。

- **Webhook 慢速 GitLab API 调用阻塞事件循环（issue #190）**：`POST /webhook/gitlab` 过去会在异步路由中直接执行同步的 `WebhookHandler.handle`，网络抖动时会阻塞健康检查和其他 API。现使用 `asyncio.to_thread` 在线程池执行处理器，并增加慢速两秒调用期间健康检查在 `200ms` 内响应的回归测试。

- **对账可重新调度人工停止的 Issue（issue #430）**：此前人工停止任务后，Issue 的最后一条评论若来自 bot，对账会沿用“最后发言人为 bot”过滤而永久跳过，导致后续手动或定时对账无法重新入队。现仅在最近任务为明确人工停止的 `interrupted` 状态时放行重新调度；bot 提问未获回复和非人工中断仍保持原有跳过行为。新增对应正反回归测试。
...
- **dsh 模型用量限制错误误归类为未知（issue #421）**：任务 #611 的 dsh
  ...
  限制归入引擎类（engine），复用已有的引擎检查、切换建议与降级语义；新增
  英文变体、与无法解决/网络错误共存时的优先级，以及失败收尾落库与评论的回归测试。
...
### Added
- **概览页面支持批量关闭 Issue（issue #412）**：开放 Issue 列表新增逐条勾选、全选当前列表和批量关闭按钮；关闭前二次确认，复用单条关闭接口的权限校验，逐条请求并归集成功/失败结果，部分失败时继续处理其余 Issue；成功项刷新并从列表移除，失败项保留并展示原因；新增空选择提示、重复提交防护、重复 Issue 去重和异常参数边界测试。
...
- **竖屏下设置页改为左右布局（issue #339）**：竖屏界面（641~860px 且
  ...
  本次竖屏下保持「设置左侧导航栏 + 设置面板」左右布局，呈现手机/平板设置页面
  「左侧列表 + 右侧详情」的主从式感觉：
  - **左右布局**：`.settings-layout` 竖屏下恢复两栏网格
    （`grid-template-columns: auto minmax(0, 1fr)`，覆盖 860px 断点的单栏
    回落），间距收紧为 16px（`--space-4`，比桌面 24px 紧凑，窄屏下给内容区
    多留空间）；
  - **紧凑吸顶导航**：`.settings-sidebar` 竖屏下恢复 sticky 吸顶（覆盖
    860px 断点的 static）并把宽度收窄为 200px（桌面 240px 更紧凑，接近手机
    设置列表密度）；`.settings-nav` 恢复内部滚动（`max-height:
    calc(100vh - 92px)` + `overflow-y: auto`，覆盖 860px 断点的
    `max-height: none`）——导航项超出视口高度时左栏独立滚动，右侧面板滚动时
    左栏列表常驻可见；搜索/分组折叠/点击定位高亮/整体折叠（issue #168）等
    既有交互不变；
  - **范围界定**：仅 641~860px 竖屏生效——内容区最宽元素（AI 供应商表格，
    实测 min-content ≈335px）+ 侧栏 200px + 间距 16px 至少需要约 551px
    视口，≤640px 手机竖屏内容列装不下两栏，保持 860px 断点单栏（issue #270
    移动端适配，375px 无横向滚动）；横屏窄视口（orientation: landscape）
    保持 860px 断点单栏（导航置顶）；桌面端（>860px）两栏布局不受影响；
  - **新增测试**：`tests/settings-portrait-layout.test.mjs` 6 例——存在针对
    设置页的竖屏断点（min-width: 641px 限定）、竖屏两栏网格、侧栏 sticky +
    200px、导航面板内部滚动、横屏窄视口单栏规则保留、桌面两栏不受影响（源码级
    防回归）；e2e `responsive-mobile.spec.js` 375px 手机竖屏设置页用例保持
    单栏断言不变，新增 768×1024 平板竖屏 describe（侧栏 sticky、宽度≈200px、
    侧栏整列位于面板左侧、无横向滚动溢出）；全量前端测试无 regression。
...
- **技能页面增加「同步所有 agent 技能」功能（issue #328）**：技能页顶部新增
  ...
  - **后端**：`botler/skills.py` 新增 `sync_skills_to_engines()`——按引擎
    注册顺序收集全部技能，同名保留注册顺序第一个版本、其余去重（返回
    merged / deduped 清单）；把去重后的技能复制到每个引擎的全部技能根目录
    （claude → `~/.claude/skills`、hermes → `$HERMES_HOME/skills`、dsh →
    `$DSH_HOME/skills` + `~/.agents/skills`，外部插件 `skills_dir` 同理）；
    目标已存在同名技能跳过不覆盖、缺失技能根目录自动创建、单技能复制失败
    记录 errors 不中断整体同步（互斥锁防并发）；新增 `POST /api/skills/sync`
    接口返回汇总统计与各引擎新增/跳过/失败明细；
  - **前端**：技能页顶部「同步所有 agent 技能」按钮（同步中禁用），点击后
    展示汇总统计（合并/去重/新增/跳过/失败）与同步明细（去重情况 + 各引擎
    技能根目录新增/跳过/失败表格），同步成功后刷新技能列表；
  - **测试**：后端新增 `TestSyncSkills`（合并去重/缺失根自动创建/跳过不覆盖/
    幂等/嵌套技能/外部插件/空池/复制失败记录，9 例）与 API 同步接口测试
    （2 例）；前端新增同步入口源码断言 + 交互统计展示 + 失败提示测试（3 例）；
    前后端全量测试无 regression。
