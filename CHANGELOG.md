...
- **识图模型测试失败时返回信息不完整：失败提示统一带上「POST 给上游 API」的请求地址与接口返回内容（issue #156）**：
  设置页「识图模型」卡片点「测试」上传图片调用识图模型，失败时前端只展示后端返回的错误文本；
  部分失败路径的信息不足以定位问题——网络层失败（超时/连接失败/DNS/SSL）只报「请求超时（>Ns）」
  或「网络请求失败: {httpx 异常}」，不含实际请求地址；接口返回 2xx 但响应缺少内容/未包含文本描述
  时只带响应片段、不带请求地址。本次修复让所有失败路径的错误提示统一包含「post 给 api」的请求
  地址（有响应时再附接口返回状态码与内容片段），用户可据此判断 POST 发到了哪个 URL、上游接口返回
  了什么内容。改动：
  - 后端 `vision_models.py` `VisionModelClient.describe()`：超时/网络异常（`httpx.TimeoutException`
    / `httpx.HTTPError`）从异常携带的 request 提取实际请求地址拼进错误信息；若异常带 response
    （如 `HTTPStatusError`）再附「接口返回: HTTP {status} {内容片段}」；
  - 后端 `plugins/vision_models.py`：Gemini / OpenAI / 自定义三个识图供应商「响应缺少内容」、
    「响应未包含文本描述」类错误补充请求地址（原有 4xx/5xx 与非 JSON 路径已带地址/内容，保持不变）；
  - 前端无需改动：失败提示展示后端 `error` 原文，`.err-hint` 样式已支持多行/长文本折行
    （issue #151 同款）；
  - **测试**：`test_vision_models.py` 新增 5 用例（超时带请求地址 / 连接失败带请求地址 /
    Gemini 缺少内容带请求地址 / OpenAI 与自定义「未包含文本描述」带请求地址）；
    `test_api_settings.py` 新增 1 用例（端点将带请求地址的错误原文完整透传给前端）。
    修复前新用例全部可复现失败（错误信息不含「请求地址」），修复后全部通过。
