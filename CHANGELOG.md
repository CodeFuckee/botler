...
- **概览页 Issue 详情抽屉在 GitLab 请求阻塞时永久加载（issue #439）**：详情请求现具有 15 秒总期限，超时后会显示可重试的明确错误，而非让评论、活动和标记活动长期停留在“加载中”。请求层同时支持调用方取消与总期限约束，并防止切换或关闭抽屉后旧请求的延迟响应覆盖当前 Issue 数据；新增超时、重试恢复和请求竞态回归测试。
