...
> 概览页「开放 Issue」板块支持**批量关闭 Issue**（issue #412）：勾选当前列表
> 中的一个或多个开放 issue，也可以使用「全选当前列表」，点击「批量关闭」后会
> 先进行二次确认，再沿用单条关闭的权限校验逐条提交；批量请求中的部分失败不会
> 阻断其他 issue，成功项会从列表移除，失败项会保留并展示 GitLab 返回的原因。
> 批量操作不跨页，也不提供撤销入口。
