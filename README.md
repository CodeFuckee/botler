...
backend/
  botler/
    ...
    api/repo_logo.py  仓库 logo 生成与同步 API（issue #188/#297）：POST /api/repos/{id}/generate-logo——agent 基于仓库 README 生成 logo 提示词 → 调用生图模型（设置页「生图模型」配置）生成 logo 落盘并写 repos 表；GET /api/repos/{id}/logo 读取图片（?download=1 触发下载）；POST /api/repos/{id}/sync-logo 把已生成 logo 上传为 GitLab 项目图标（头像；上传前自动压缩到 200KB 以内，issue #310）
...
POST   /api/repos/{id}/sync-logo   仓库管理页「同步到 GitLab」按钮（issue #297）：把已生成 logo 上传为 GitLab 项目图标（头像）——读取本地 logo 文件，经 GitLab API PUT /projects/{id} 的 avatar 文件参数上传；身份复用 issue 创建链路（仓库 remote URL 内嵌 token 优先，无 token 回退全局 bot token）；上传前用 Pillow 把超限图片压缩到 200KB 以内（GitLab 项目头像上限，issue #310），本地 logo 文件保持原始质量；成功返回 ok + 项目 path_with_namespace + avatar_url
