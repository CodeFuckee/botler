...
> 💡 **概览页手动执行 Issue**（issue #431）：打开任一开放 Issue 的右侧详情栏后，若该 Issue 当前没有 `queued`、`running` 或 `retrying` 任务，可点击“执行”并确认，平台会新建一条 `manual` 来源的任务后立即交给现有调度器入队（同仓库仍遵守串行调度，因此“立即”指立即入队而非绕过队列并发执行）。已关闭的 Issue 或已有活跃任务时按钮隐藏；失败任务的“重试”仍保留原来的断点续跑语义。
...
> 💡 **维护模式**（issue #241）：可在设置页「任务调度」卡片一键开启/恢复——
> 紧急情况（服务器负载高 / GitLab 故障 / 停服维护 / 误配全仓触发）下，开启后
> 调度器**立即停止开始新任务**（webhook 事件照常接收、对账照常扫描），
> **运行中任务继续执行完、不中断**，关闭后自动恢复派发（队列中任务自动开始）；
> 维护期间新事件默认**只入队不派发**（恢复后自动执行），也可配置为直接忽略
> （不建任务）。导航栏「维护中」徽章与概览页顶部横幅实时提示状态；开启与
> 恢复各记录一条通知事件。设置保存即生效（config `worker.maintenance_mode` +
> 内存态），无需重启服务。
...
> 任务详情。
>
> ⚠️ **大模型 API 请求错误不自动上报**（issue #494）：任务失败若由大模型
> API 请求错误导致（LLM 供应商限流 / 过载 / 服务不可用 / 请求失败等，命中
> `auto_issue` 插件的 `LLM_API_ERROR_PATTERNS` 规则），跳过失败上报——该
> 错误属平台侧共因（一个供应商故障影响所有任务），逐任务上报只会刷屏，且
> 重试后大概率自愈，无需人工逐个介入；GitLab 侧 401/403/429 等错误不受
> 影响，仍正常上报。
...
backend/
  botler/
    ...
    api/repo_logo.py  仓库 logo 生成与同步 API（issue #188/#297/#320）：POST /api/repos/{id}/generate-logo——agent 基于仓库 README 生成 logo 提示词 → 调用生图模型（设置页「生图模型」配置）生成 logo 落盘并写 repos 表；GET /api/repos/{id}/logo 读取图片（?download=1 触发下载；?thumb=1 返回小尺寸缩略图预览——issue #338 仓库页列表低网速快速加载）；POST /api/repos/{id}/sync-logo 把已生成 logo 上传为 GitLab 项目图标（头像；上传前自动压缩到 200KB 以内并归一为 GitLab 支持格式，issue #310/#318）；POST /api/repos/{id}/sync-logo-from-gitlab 反向同步——把 GitLab 项目当前图标（头像）拉回落盘为本地仓库 logo（issue #320，双向同步闭环）
    ...
    api/stats.py     统计看板 API（issue #264/#274/#224）：GET /api/stats/dashboard——本地任务表聚合（任务总数/成功率/平均耗时/失败数 + 按引擎/仓库/来源分组 + 按来源×日期逐日趋势 by_source_daily（issue #224，统计页「来源分布」卡片——issue #361 由概览页迁入——与统计页「来源按天趋势」数据源）+ 失败原因 Top（附分类）+ 失败原因分类分布），10 秒 TTL 缓存，与任务列表同表同口径
...
> 💡 **「添加 Issue」弹窗滚动布局**（issue #458）：弹窗内容超过可视高时，头部
> 标题固定在顶部、取消/创建 Issue 按钮固定在底部，中间的标题、描述、图片附件、
> 分配人、标签等表单区域独立上下滚动（`.modal.add-issue` 采用 flex 列布局 +
> `.add-issue-body` 滚动容器，滚动不穿透到底层页面）。
...
> 开放 issue，也可以使用「全选当前列表」。确认关闭前会二次确认，再沿用单条关闭的权限
> 校验逐条提交；批量请求中的部分失败不会阻断其他 issue，成功项会从列表移除，失败项会
> 保留并展示 GitLab 返回的原因。选择模式可随时取消，取消会清空本次选择；批量操作不跨页，
> 也不提供撤销入口。
...
> 概览页「开放 Issue」板块支持**切换展示布局**（issue #471）：过滤条顶部
> 提供「仓库卡片 / 单列分组」两种布局——「仓库卡片」为默认布局（每个仓库
> 一张卡片，卡片内按 bot 状态分组，宽屏多列网格）；「单列分组」让所有仓库
> 的 issue 在同一列展示，先按 issue 状态分组——进行中（running）第一、
> 完成任务（bot-done）第二、失败任务（bot-failed）第三、其他任务（other）
> 第四（issue #485），状态组内再按仓库分组；仓库子分组头部带
> **折叠/展开开关**（折叠后隐藏组内 issue 列表、保留组头仓库名/优先级/
> 计数/操作按钮），折叠偏好持久化到浏览器本地（localStorage 键
> `botler.overview.collapsedRepos`），刷新后保持；布局偏好同样持久化
> （localStorage 键 `botler.overview.layout`）。两种布局共用过滤 / 排序 /
> 批量关闭 / 置顶能力，排序与过滤结果在「单列分组」下按「状态 → 仓库」两级
> 分组原样展示；置顶按钮仅在「其他」状态组内提供（与卡片布局语义一致）。

> 界面语言（issue #268）：左侧边栏底部与设置页「界面显示」卡片提供「中文 / English」快捷切换，
> 选择持久化到浏览器本地（localStorage 键 botler.lang），刷新后保持；高频页面（导航 / 概览 /
> 任务）静态文案即时切换，未翻译文案回退中文；后端错误消息与动态内容保持原文。
>
> 界面显示（issue #342）：设置页「界面显示」卡片提供「合并显示评论与活动（时间线）」开关
...
> 时出现，点击平滑回到顶部；系统开启「减弱动态效果」时改为直接跳转。
> 页面内容不足一屏或位于顶部附近时不显示，避免遮挡；版本更新横幅显示时
> 按钮自动上移让位。

> 右侧边栏抽屉「回到顶部」按钮（issue #457）：所有需要竖向滚动的右边栏
> 抽屉（issue 详情 / 流水线详情 / 任务执行详情 / 灵感 AI 对话 / 任务详情
> 快览）右下角同样提供「回到顶部」浮动按钮——滚动抽屉内容超过约两屏
> （400px）时出现，点击平滑滚动抽屉到顶部；按钮以 `position: sticky`
> 钉在抽屉可视区右下角（不随内容滚出可视区），移动端 issue 抽屉底部操作栏
> 常驻时自动上移让位。
...
**插件管理页（issue #145）**：左侧边栏「插件」入口（`/plugins`）集中管理
所有插件的安装、卸载与设置——
...
左侧边栏「技能」入口（`/skills`）展示**所有配置的执行引擎（executor 插件）
...
左侧边栏「工具」入口（`/tools`）管理**给 agent 使用的 MCP 工具**，全局
...
浏览器内直接使用系统终端（左侧边栏「终端」→ `/terminal`），**无需再打开系统终端**：
...
| `inspection.enabled` | true | 仓库健康巡检总开关（issue #265；关闭后定时巡检不执行，手动重检仍可用） |
| `inspection.interval_seconds` | 21600 | 仓库健康巡检间隔（秒，默认 6 小时；下限 300） |
| `inspection.auto_repair` | true | webhook 缺失 / secret 不匹配时自动重新注册（issue #265） |
...
| `worker.maintenance_mode` | `false` | 维护模式（issue #241）：人工总开关——开启后调度器立即停止开始新任务（webhook 事件照常接收、对账照常扫描），运行中任务继续执行完，关闭后自动恢复派发。设置页「任务调度」卡片可一键切换（写回 config + 内存态，保存即生效，无需重启服务） |
| `worker.maintenance_hold_events` | `true` | 维护模式下新事件处理方式（issue #241）：`true` = 任务只入队不派发（webhook/对账照常建任务，关闭维护模式后自动执行，默认）；`false` = 直接不建任务（webhook 事件忽略、对账扫描但跳过创建） |
...
| `ai_providers[]` | 空列表 | AI API 供应商配置（设置页「AI API 供应商」卡片增删改查，issue #46）：每项 `{name, provider, base_url, api_key, model, enabled, priority}`，内置 DeepSeek / OpenAI / Anthropic / Gemini / Moonshot / 通义千问 / 智谱 / 硅基流动 / Ollama / OpenRouter 预设；api_key 落盘 config.yaml（支持 `${ENV}` 引用），API 只返回掩码。设置页编辑表单提供**「获取模型」按钮**（issue #499）：经后端代理 `POST /api/ai/list-models` 调 OpenAI 兼容 `GET {base_url}/models` 拉取该供应商全部可用模型供选择，点击模型即填入「默认模型」；API Key 留空 / 掩码时按 name 匹配已保存配置。priority 为 1~999 整数（issue #495：**数字小优先级高**，缺省 100，与仓库调度优先级 `repos[].priority` 同语义，issue #51）：调用 AI（灵感对话 / 仓库发掘 / 仓库自省 / 生成图标 / dsh 凭据回退）时优先使用启用的高优先级供应商，该供应商**额度不足 / 调用失败后自动切换**下一个启用的供应商，全部失败返回逐供应商汇总错误（便于诊断配置）；灵感对话显式指定某供应商时保持「硬选择」不自动切换。dsh 引擎未配 api_key 时回退 provider=deepseek 且启用的项（issue #115）；概览页「DeepSeek 账户余额」（issue #138）同样按此链解析 Key（dsh 段 > AI 供应商 deepseek 项 > 环境变量 `DEEPSEEK_API_KEY`）代调 `GET https://api.deepseek.com/user/balance` 展示余额；余额卡片提供「去充值」链接按钮，一键跳转 DeepSeek 开放平台充值页（`https://platform.deepseek.com/top_up`，issue #178）；余额卡片每个币种条目展示「每小时余额变化速率」（issue #304：纯前端基于 localStorage 观测样本按最早/最近窗口计算每小时平均变化，减少为负/增加为正/无变化为 0，样本不足时展示「暂无速率数据」） |
...
POST   /api/repos/{id}/generate-logo 仓库管理页「生成图标」按钮（issue #188）：agent 基于该仓库 README（本地文件夹优先，GitLab 仓库 API 兜底，均缺失时基于仓库元信息）生成 logo 提示词 → 调用生图模型（设置页「生图模型」第一个启用且 Key 非空的项）生成 logo，首张落盘 LOGO_DIR（BOTLER_LOGO_DIR 显式覆盖 → BOTLER_DATA_DIR/backend/data/logos 持久目录 → backend/data/logos 兜底，issue #309 保证不随构建目录轮换丢失）并写 repos 表 logo_path / logo_updated_at / logo_mime；重复点击覆盖重生成；**未启用仓库同样可生成图标（issue #323，停用只影响任务调度，不限制设置页 logo 操作，与 sync-logo / sync-logo-from-gitlab 行为一致）**
GET    /api/repos/{id}/logo         读取该仓库已生成的 logo 图片（img src 直连，前端按 logo_updated_at 拼缓存击穿参数）；?download=1 返回 Content-Disposition attachment 供下载；?thumb=1 返回小尺寸缩略图预览（issue #338：Pillow 内存等比缩放到最长边 ≤ 96px，列表直接加载缩略图、放大弹窗加载原图，低网速下列表快速可见；Pillow 缺失/SVG/损坏文件回退原图，download 优先于 thumb）
POST   /api/repos/{id}/sync-logo   仓库管理页「同步到 GitLab」按钮（issue #297）：把已生成 logo 上传为 GitLab 项目图标（头像）——读取本地 logo 文件，经 GitLab API PUT /projects/{id} 的 avatar 文件参数上传；身份复用 issue 创建链路（仓库 remote URL 内嵌 token 优先，无 token 回退全局 bot token）；上传前用 Pillow 把超限图片压缩到 200KB 以内、把不支持格式（如 WebP）转码为 GitLab 支持格式（png/jpeg/gif/bmp/tiff/vnd.microsoft.icon，含透明转 PNG，issue #310/#318），本地 logo 文件保持原始质量；成功返回 ok + 项目 path_with_namespace + avatar_url
POST   /api/repos/{id}/sync-logo-from-gitlab 仓库管理页「从 GitLab 同步」按钮（issue #320）：把 GitLab 项目当前图标（头像）同步回仓库设置页——经 GitLab API GET /projects/{id} 取 avatar_url、GET /projects/{id}/avatar 下载图标字节（uploads 直链需会话鉴权，不能直接用 avatar_url 下载），落盘 LOGO_DIR 并写 repos 表 logo_path / logo_updated_at / logo_mime，仓库列表最左侧即展示 GitLab 图标（可放大/下载/继续「同步到 GitLab」回写，双向同步闭环）；GitLab 侧未设置图标返回 400 引导先去 GitLab 上传；身份复用 issue 创建链路（仓库 remote URL 内嵌 token 优先，无 token 回退全局 bot token）
...
GET    /api/stats/dashboard           统计看板（issue #264/#274/#224）：本地任务表聚合——overview（任务总数/成功率/平均耗时/失败数）+ by_engine/by_repo/by_source（分组对比，含成功率/平均耗时）+ by_source_daily（按来源×日期逐日趋势，days>0 最近 N 天逐日零填充、days=0 仅返回有数据日期，issue #224——统计页「来源分布」卡片（issue #361 由概览页迁入）与统计页「来源按天趋势」数据源，前端随 7/30 天时间段选择联动）+ failure_reasons（失败原因 Top 10，failed/interrupted 的 error_message 归一化，每条附 category/category_name 失败分类）+ failure_categories（失败原因分类分布，落库值优先、旧任务实时分类兜底）；days 参数 0=全部/N=最近 N 天，10 秒 TTL 缓存
...
- **统一日志脱敏（issue #259）**：新增 `botler/log_redact.py`——写入日志前对
  ...
  错误响应、全部 logging 输出统一走脱敏；正常日志内容不受影响。
