...
class TestMigrateInspirations:
    ...
    def test_old_db_gets_inspirations_table(self, tmp_path):
        ...
        assert ver == 32, f"user_version 应推进到 32（v22 engine_fallback；v23 仓库级任务参数；v24 预检结果；v25 token_expires_at；v26 audit_logs；v27 repo_health；v28 灵感对话供应商；v29 灵感标签/归档/issue 关联列；v30 模板版本历史表 template_versions；v31 通知已读 read_at 列；v32 repo_health.project_detail），实际 {ver}"
...
class TestMigrateInspirationChatProvider:
    """issue #249：旧灵感表补 chat_provider 持久化列。"""

    def test_old_db_gets_chat_provider_column(self, tmp_path):
        path = tmp_path / "old249.db"
        _build_old_db(path)
        conn = sqlite3.connect(str(path))
    ...
    def test_new_db_persists_chat_provider_and_clear(self, tmp_path):
        ...
        repo = db.upsert_repo(249, "demo", "https://example.com/demo.git")
        inspiration = db.create_inspiration(repo, "选择模型")
        assert db.set_inspiration_chat_provider(inspiration, "gemini")
        assert db.get_inspiration(inspiration)["chat_provider"] == "gemini"
...
class TestMigrateTemplateVersions:
    """v30 迁移（issue #262）：模板版本历史表 template_versions。"""

    def test_old_db_gets_template_versions_table(self, tmp_path):
        """旧库（user_version=29，无 template_versions 表）初始化后应补出该表。"""
        path = tmp_path / "old262.db"
        conn = sqlite3.connect(str(path))
  ...
  created_at TEXT DEFAULT (datetime('now'))
        ...
        assert ver == 32, f"user_version 应推进到 32（v30 模板版本历史表；v31 通知已读 read_at 列；v32 repo_health.project_detail），实际 {ver}"

    def test_new_db_has_template_versions_table(self, tmp_path):
        """新库建表语句应直接含 template_versions 表（无需迁移）。"""
        db = Database(str(tmp_path / "new262.db"))
        with db._conn() as conn:
            tables = {r["name"] for r in conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table'")}
            cols = {r["name"] for r in conn.execute(
                "PRAGMA table_info(template_versions)")}
        assert "template_versions" in tables
        assert {"id", "template_key", "version_no", "content",
                "note", "created_at", "updated_at"} <= cols

    def test_template_versions_crud_after_migration(self, tmp_path):
        """迁移后的库可直接读写模板版本（版本记录/列表/回滚数据源）。"""
        db = Database(str(tmp_path / "crud262.db"))
        vid = db.record_template_version("global:default", "模板")
        assert vid is not None
        rows = db.list_template_versions("global:default")
        assert len(rows) == 1
        assert rows[0]["content"] == "模板"
