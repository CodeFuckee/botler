...
class TestMigrateInspirations:
    ...
    def test_old_db_gets_inspirations_table(self, tmp_path):
        ...
        assert ver == 24, f"user_version 应推进到 24（v22 engine_fallback，issue #236；v23 repos 仓库级任务参数覆盖 timeout_seconds/max_retries/engine，issue #237；v24 tasks.precheck_result 预检结果列，issue #238），实际 {ver}"
...
class TestMigrateInspirationMessages:
    ...
    def test_old_db_gets_inspiration_messages_table(self, tmp_path):
        ...
        assert ver == 24, f"user_version 应推进到 24（v22 engine_fallback，issue #236；v23 repos 仓库级任务参数覆盖，issue #237；v24 tasks.precheck_result 预检结果列，issue #238），实际 {ver}"
