...
class TestMigrateInspirations:
    ...
    def test_old_db_gets_inspirations_table(self, tmp_path):
        ...
        assert ver == 34, f"user_version 应推进到 34（…v31 通知已读 read_at 列；v32 repo_health.project_detail；v33 zcode 会话 zcode_session_id；v34 远程项目 remote_host/remote_path），实际 {ver}"
...
class TestMigrateTemplateVersions:
  ...
  created_at TEXT DEFAULT (datetime('now'))
        ...
        assert ver == 34, f"user_version 应推进到 34（v30 模板版本历史表；…v32 repo_health.project_detail；v33 zcode 会话 zcode_session_id；v34 远程项目 remote_host/remote_path），实际 {ver}"
