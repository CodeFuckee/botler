...
- **响应反馈**：按钮按下 `translateY(1px) scale(0.98)` 微缩放；可交互行
  ...
  hover 轻抬升 + 阴影加深；可关闭提示条 hover 阴影加深 + 按下微缩放；
  任务详情「思考过程」折叠条 hover 变主色；标签多选胶囊 hover 微降透明度

## 8. 交互规范

| 状态 | 通用规则 |
|---|---|
| hover | 可交互元素必须有可见反馈：按钮变灰底、链接变主色、输入框边框加深、列表行灰底、提示条阴影加深、思考折叠条变色、标签胶囊透明度变化 |
| active | 按下即时反馈：按钮 `translateY(1px) scale(0.98)` 微缩放；链接/折叠标题/关闭按钮/选项胶囊 opacity/背景变化、提示条 `scale(0.99)`（反馈活在按下瞬间） |
