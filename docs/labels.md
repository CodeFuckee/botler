...
| 标签 | 颜色 | 说明 |
|---|---|---|
| `bot-issue` | `#6699cc` | 调度排最后（任务带此标记时排在**所有任务之后**，issue #360；设置页「任务调度 → issue 标签优先级」显式包含 `bot-issue` 时可自定义顺序） |
...
2. **优先级判定**（GitLab Issue Agent）：默认 `bug` > `test` > `feature`（同仓库队列内，bug 最优先）；顺序可在平台设置页「任务调度 → issue 标签优先级」自定义，未列出的标签排在最后；带 `bot-issue` 标记的任务排在**所有任务之后**（除非配置中显式包含 `bot-issue` 自定义顺序）；同优先级按 issue 更新时间升序处理。
