...
function renderList(lines, keyPrefix = '') {
  const rootItems = []
  let i = 0
  while (i < lines.length) {
    ...
    rootItems.push(
      <li key={`${keyPrefix}li${i}`}>
        ...
        {sub.length > 0 && renderList(sub, `${keyPrefix}li${i}-`)}
      </li>
    )
    i++
  }
...
function renderTable(lines, keyPrefix = '') {
  ...
  return (
    <table className="table" key={`${keyPrefix}tbl`}>
      <thead>
        <tr>{header.map((c, j) => <th key={`${keyPrefix}h${j}`}>{renderInline(c, `${keyPrefix}h${j}-`)}</th>)}</tr>
      </thead>
      <tbody>
        {body.map((r, ri) => (
          <tr key={`${keyPrefix}r${ri}`}>
            {r.map((c, j) => <td key={`${keyPrefix}r${ri}c${j}`}>{renderInline(c, `${keyPrefix}r${ri}c${j}-`)}</td>)}
          </tr>
        ))}
      </tbody>
    </table>
...
export default function Markdown({ content }) {
  ...
  return (
    <div>
      {blocks.map((b, i) => {
        switch (b.type) {
          case 'h1': return <h1 key={i}>{renderInline(b.text, `b${i}-`)}</h1>
          case 'h2': return <h2 key={i}>{renderInline(b.text, `b${i}-`)}</h2>
          case 'h3': return <h3 key={i}>{renderInline(b.text, `b${i}-`)}</h3>
          case 'h4': return <h4 key={i}>{renderInline(b.text, `b${i}-`)}</h4>
          case 'code': return (
            <pre key={i}><code>{b.text}</code></pre>
          )
          case 'quote': return (
            <blockquote key={i}>
              {b.lines.map((l, j) => <p key={j}>{renderInline(l, `b${i}q${j}-`)}</p>)}
            </blockquote>
          )
          case 'table': return renderTable(b.lines, `b${i}-`)
          case 'list': return renderList(b.lines, `b${i}-`)
          case 'p': return <p key={i}>{renderInline(b.text, `b${i}-`)}</p>
          default: return null
        }
      })}
    </div>
