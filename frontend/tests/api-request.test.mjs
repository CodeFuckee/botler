...
// ---- 可选 deadline / 调用方取消（issue #439）----

test('GET 指定 timeoutMs：挂起请求被取消并抛出超时错误', async () => {
  let seenSignal = null
  const m = installFetch((_i, _url, opts) => new Promise((_resolve, reject) => {
    seenSignal = opts.signal
    opts.signal.addEventListener('abort', () => reject(opts.signal.reason))
  }))
  try {
    await assert.rejects(
      () => api.get('/api/issues/42/97/detail', { timeoutMs: 20, silent: true }),
      (err) => err?.code === 'REQUEST_TIMEOUT')
    assert.ok(seenSignal?.aborted, 'deadline 到期应取消 fetch')
    assert.equal(m.calls.length, 1, '超时后不应再尝试重试')
    assert.equal(currentToasts().length, 0, 'silent 超时不应弹 toast')
  } finally {
    m.restore()
  }
})

test('GET 指定外部 signal：调用方取消透传且不被误判为超时', async () => {
  ...
  try {
    const pending = api.get('/api/issues/42/97/detail', {
      timeoutMs: 1000, signal: controller.signal, silent: true,
    })
    controller.abort()
    await assert.rejects(pending, (err) => err?.code !== 'REQUEST_TIMEOUT')
    assert.ok(seenSignal?.aborted, '调用方取消应传至 fetch')
    assert.equal(m.calls.length, 1, '调用方取消不应重试')
  } finally {
    m.restore()
  }
})
