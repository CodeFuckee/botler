// 概览页三区块列数统一测试（issue #107）：开放 Issue、正在执行任务、
// CI/CD 流水线三个板块的网格列数应统一，以开放 Issue 板块为标准
...
//
// 断言：
// 1. styles.css：三个板块容器（.issues-list / .overview-grid /
//    .pipelines-list）均使用 repeat(auto-fit, minmax(Npx, 1fr))；
// 2. 三者的 minmax 最小列宽与网格间距完全一致（以开放 issue 板块为标准，
//    一处改动不得漂移）；
// 3. .overview-grid / .pipelines-list 不再使用固定 3 列（防回退）；
// 4. 列数模拟：相同视口宽度下三个板块的 auto-fit 列数一致（宽屏 ≥4 列、
//    窄视口自动降列不溢出）；
// 5. 渲染级：多任务 / 多仓库流水线渲染的卡片数不丢卡（auto-fit 布局
//    与卡片渲染解耦，布局改动不丢数据）。
...
// 概览页三板块网格容器：开放 Issue（标准）/ 正在执行任务 / CI/CD 流水线
const GRID_SELECTORS = ['.issues-list', '.overview-grid', '.pipelines-list']
...
test('三板块容器均使用 auto-fit 自适应网格（以开放 Issue 板块为标准）', () => {
...
test('.overview-grid / .pipelines-list 不再使用固定 3 列（防回退）', () => {
  for (const sel of ['.overview-grid', '.pipelines-list']) {
    const spec = gridSpec(styles, sel)
    assert.doesNotMatch(
      spec.columns || '',
      /repeat\(3,\s*1fr\)/,
      `${sel} 不应再使用固定 3 列（与开放 Issue 板块列数不一致）`,
    )
  }
})

test('三板块 minmax 最小列宽与网格间距完全一致（一处改动不得漂移）', () => {
...
test('列数模拟：相同视口下三板块 auto-fit 列数一致（宽屏 ≥4、窄屏降列不溢出）', () => {
  const std = gridSpec(styles, '.issues-list')
  ...
  for (const vp of [1000, 1120, 1280, 1360, 1440, 1540, 1920, 2542, 2560]) {
    ...
    // 宽屏（≥1280 可用宽度下开放 issue 板块可容 ≥4 列）三板块同列数
    if (vp >= 1280) {
...
test('渲染：多任务卡片不丢卡（auto-fit 布局与渲染解耦）', async () => {
  const { renderer, renderError } = await renderOverview({ tasks: [1, 2, 3, 4, 5].map(mkTask) })
  try {
    assert.equal(renderError, null, `渲染抛错：${renderError?.message || renderError}`)
    assert.equal(countCards(renderer, 'overview-grid'), 1, '应渲染 overview-grid 容器')
    assert.equal(countCards(renderer, 'overview-card'), 5, '5 个任务应渲染 5 张任务卡片')
  } finally {
