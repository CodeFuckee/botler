// 概览页响应式布局测试（issue #96）：宽度足够时开放 issue 板块的
...
// 两边各留 480px 空白。
// 修复后：.issues-list 自适应列数（auto-fit + minmax(280px, 1fr)），
// 宽度足够时 4 卡一行，仓库更多/更少时自动增减列；宽屏断点
// ≥1920→1840px、≥2560→2480px，两边各留约 40px 边距。
//
// 断言：
// 1. styles.css：.issues-list 使用 auto-fit 自适应网格，列最小宽 280px；
// 2. styles.css：宽屏断点 ≥1920→1840、≥2560→2480（2K 屏边距 ~40px）；
// 3. 模拟 auto-fit 列数：1280/1360/1440/1920/2560 视口下可容 ≥4 列
//    （4 卡一行），且每列宽 ≥280 不挤压；窄视口（1000/1120）自动降为
//    3 列回退；
// 4. Tasks.jsx contentWidthAt 与 CSS 宽屏断点同步（新值显式断言防回退）；
...
test('styles.css：宽屏断点 ≥1920→1840、≥2560→2480，两旁仅留约 40px 边距', () => {
  const breaks = contentBreakpoints(styles)
  const byMin = Object.fromEntries(breaks.map((b) => [b.min, b.width]))
  // 1920 视口：内容区 1840 = 视口 − 80（两边各 40px）
  assert.equal(byMin[1920], 1840, '≥1920 视口内容区应放宽至 1840px（两边各留 40px）')
  // 2K（2560）视口：内容区 2480 = 视口 − 80（两边各留 40px）
  assert.equal(byMin[2560], 2480, '≥2560 视口内容区应放宽至 2480px（两边各留 40px）')
...
test('auto-fit 列数：宽屏视口下 4 个仓库一行放下，每列宽 ≥280 不挤压', () => {
  ...
  for (const vp of [1280, 1360, 1440, 1920, 2560]) {
...
// ---- contentWidthAt 与 CSS 断点同步（新值显式断言，防回退到 1600px）----

test('contentWidthAt：与 styles.css 宽屏断点同步（≥1920→1840、≥2560→2480）', () => {
  assert.equal(contentWidthAt(1920), 1840, '1920 视口内容区应为 1840px')
  assert.equal(contentWidthAt(1919), 1440, '1920 以下仍为 1440px 档')
  assert.equal(contentWidthAt(2560), 2480, '2560 视口内容区应为 2480px')
  assert.equal(contentWidthAt(2559), 1840, '2560 以下仍为 1840px 档')
  // 超大视口不越界：封顶 2480
  assert.equal(contentWidthAt(4000), 2480, '超大视口应封顶 2480px')
  // 与 CSS 断点推导动态对比（覆盖新断点）
  const breaks = contentBreakpoints(styles)
  for (const vp of [1280, 1440, 1919, 1920, 2559, 2560, 4000]) {
    assert.equal(
      contentWidthAt(vp),
