// 概览页「来源分布」统计卡片测试（issue #224）：
// 概览页底部新增来源分布统计卡片——按任务来源（webhook/手动/对账）展示
// 任务量、成功率与平均耗时（近 30 天）。数据来自本地任务表聚合接口
// GET /api/stats/dashboard?days=30（后端 10s TTL 缓存，与任务列表同表
// 同口径，无 GitLab 请求压力），组件自持 60 秒低频轮询（usePolling，
// 页面隐藏自动暂停，后台 0 请求）。
//
// 断言：
// 1. Overview.jsx 引入并渲染 SourceStatsSection；
...
const sectionSrc = readFileSync(path.join(ROOT, 'src/components/overview/SourceStatsSection.jsx'), 'utf8')
...
const { default: SourceStatsSection } =
  await vite.ssrLoadModule('/src/components/overview/SourceStatsSection.jsx')
...
test('源码：Overview 引入并渲染来源分布卡片（issue #224）', () => {
  assert.match(overviewSrc, /SourceStatsSection/, '应引入来源分布组件')
  assert.match(overviewSrc, /<SourceStatsSection \/>/, '应在概览页渲染来源分布组件')
  assert.match(overviewSrc, /issue #224/, '应有 issue #224 注释说明')
})
...
test('源码：组件请求统计看板接口（近 30 天）并 60 秒轮询', () => {
  ...
  assert.match(sectionSrc, /SOURCE_STATS_DAYS\s*=\s*30/, '概览页默认窗口应为近 30 天')
...
test('源码：文案经 i18n 国际化且中文文案保留', () => {
  assert.match(sectionSrc, /tr\('overview\.sourceStatsTitle'\)/, '标题应经 t() 国际化')
  assert.equal(zhCN['overview.sourceStatsTitle'], '来源分布', '中文标题应为「来源分布」')
  assert.match(sectionSrc, /tr\('overview\.sourceStatsDesc'/, '说明应经 t() 国际化')
  assert.match(sectionSrc, /tr\('overview\.sourceTaskCount'/, '任务量文案应经 t() 国际化')
  assert.equal(zhCN['overview.sourceTaskCount'], '{n} 个任务', '中文任务量文案应保留')
  assert.match(sectionSrc, /tr\('overview\.sourceSuccessRate'\)/, '成功率文案应经 t() 国际化')
  assert.equal(zhCN['overview.sourceSuccessRate'], '成功率', '中文成功率文案应为「成功率」')
  assert.match(sectionSrc, /tr\('overview\.sourceAvgDuration'/, '平均耗时文案应经 t() 国际化')
  assert.match(sectionSrc, /tr\('overview\.sourceStatsEmpty'\)/, '空态文案应经 t() 国际化')
})

test('styles.css 提供来源分布卡片样式', () => {
  assert.match(styles, /\.overview-source-stats-section\s*\{/, '应有板块容器样式')
  assert.match(styles, /\.overview-source-stats-cards\s*\{/, '应有卡片网格样式')
  assert.match(styles, /\.overview-source-stats-card\s*\{/, '应有卡片样式')
  assert.match(styles, /\.overview-source-stats-count\s*\{/, '应有任务量数字样式')
  assert.match(styles, /\.overview-source-stats-rate\s*\{/, '应有成功率样式')
})
