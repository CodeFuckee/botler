...
test('仓库编辑弹窗不再展示任务超时配置', async () => {
  mockApi()
  const { renderer } = await renderRepos()
  try {
    await click(renderer, findButtons(renderer, '设置')[0])
    assert.doesNotMatch(modalText(renderer), /任务超时|7200/, '执行引擎不限时，不应保留超时输入')
  } finally {
    await TestRenderer.act(() => renderer.unmount())
    mock.restoreAll()
  }
...
test('保存任务参数：PUT 仅携带重试和引擎配置', async () => {
  mockApi()
  const { renderer } = await renderRepos()
  let putBody = null
  ...
  try {
    await click(renderer, findButtons(renderer, '设置')[0])
    const { retries, engine } = findParamInputs(renderer)
    await change(renderer, retries, '5')
    await change(renderer, engine, 'HERMES')
    await click(renderer, findButtons(renderer, '保存')[0])
    assert.equal('timeout_seconds' in putBody, false, '不应提交已移除的超时配置')
    assert.equal(putBody.max_retries, 5)
    assert.equal(putBody.engine, 'hermes')
  } finally {
    await TestRenderer.act(() => renderer.unmount())
    mock.restoreAll()
  }
