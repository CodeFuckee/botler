...
function fakeSettingsContent() {
  ...
  return {
    querySelectorAll: () => [
      ...
      section('settings-auto-issue', '任务失败自动上报'),
      group('执行引擎'),
      ...
      section('settings-audit-logs', '审计日志'),
      group('账号与安全'),
...
test('每个设置区块带锚点 id + settings-section 类（导航读取的固定约定）', () => {
  const ids = ['settings-sso', 'settings-ai-providers', 'settings-image-models',
    ...
    'settings-alerts', 'settings-webhook', 'settings-claude', 'settings-dsh', 'settings-environment',
...
test('collectSettingsGroups：从设置页 DOM 结构生成 6 组 19 项（含识图模型）', () => {
  ...
  assert.equal(items.length, 19, '应有 19 个设置子项')
...
test('不变式：设置页每个设置区块都出现在导航中（不重不漏）', () => {
  ...
  const sectionIds = [...settings.matchAll(/<section id="(settings-[a-z-]+)" className="settings-section"[^>]*>/g)]
    .map((m) => m[1])
  assert.ok(sectionIds.length >= 16, `应解析到不少于 16 个设置区块，实际 ${sectionIds.length}`)
  const groups = collectSettingsGroups(fakeSettingsContent())
...
test('渲染：默认展示全部分组与子项（含识图模型），分组头可见', () => {
  ...
  try {
    ...
    assert.equal(linkCount(root), 19, '应渲染 19 个子项链接')
...
test('渲染：清空搜索恢复全部分组', () => {
  ...
  try {
    ...
    assert.equal(linkCount(root), 19, '清空搜索后应恢复全部分组子项')
  } finally {
    TestRenderer.act(() => renderer.unmount())
    restore()
  }
...
test('渲染：点击分组头折叠子项，再次点击展开', () => {
  ...
  try {
    ...
    assert.equal(linkCount(root), 19 - 5, '收起「外部服务接入」后应少 5 个子项')
    TestRenderer.act(() => { head.props.onClick() })
    assert.equal(head.props['aria-expanded'], true, '再次点击后分组应展开')
    assert.equal(linkCount(root), 19, '展开后应恢复 19 个子项')
  } finally {
    TestRenderer.act(() => renderer.unmount())
    restore()
  }
...
test('渲染：「全部收起」后所有分组折叠，「全部展开」恢复', () => {
  ...
  try {
    ...
    assert.equal(linkCount(root), 19, '全部展开后应恢复 19 个子项')
  } finally {
    TestRenderer.act(() => renderer.unmount())
    restore()
  }
