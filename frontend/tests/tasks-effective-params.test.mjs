// 任务列表/详情展示执行引擎不限时，以及实际生效的重试/引擎与来源（issue #424）。
//
// 断言：抽屉与详情页均展示「未限制」；重试与引擎继续显示其配置来源。
...
function mkTask(overrides = {}) {
  return {
    ...
    timeout_seconds: null, timeout_source: null,
...
test('任务抽屉展示生效参数与来源（仓库覆盖）', async () => {
  ...
  try {
    ...
    assert.match(text, /未限制/, '应展示执行引擎未限制')
...
test('任务抽屉展示生效参数与来源（继承全局）', async () => {
  ...
  try {
    ...
    assert.equal(drawers.length, 1, '应弹出抽屉')
    const text = textOf(drawers[0])
    assert.match(text, /未限制/, '执行引擎应展示未限制')
